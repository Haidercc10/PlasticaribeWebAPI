        ...
        public ActionResult GetOrderd(DateTime startDate, DateTime endDate, string? order = "", string? clientId = "", string? salesId = "")
        {
            ...
            var infoDev = from dev in _context.Set<Devolucion_ProductoFacturado>()
                          where dev.DevProdFact_Fecha >= startDate &&
                                ...
                                (clientId != "" ? dev.Cli_Id == Convert.ToInt32(clientId) : dev.Cli_Id.ToString().Contains(clientId)) &&
                                (salesId != "" ?  dev.Asesor_Id == Convert.ToInt32(salesId) : dev.Asesor_Id.ToString().Contains(salesId))
                          select new
                          {
                              or = new
                              {
                              ...
                              Type = "DV",
                              Estado = dev.Estado_Id == 11 ? "PENDIENTE" : dev.Estado_Id == 29 ? "EN REVISIÓN" :  dev.Estado_Id == 3 ? "ANULADO" : dev.Estado_Id == 38 ? "POR REPONER" : dev.Estado_Id == 39 ? "EN REPOSICION" : dev.Estado_Id == 18 ? "CERRADA" : dev.Estado_Id == 53 ?  "PRE-DEVUELTO" : dev.Estado_Id == 54 ? "POR FINALIZAR" : "",
